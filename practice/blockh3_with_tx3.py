...
# 模擬：
#   假設 host3 為可疑的惡意來源，當其傳送的資料量過多時將其封鎖
# 測試：
#   mininet端輸入 h3 ping h1 -s 200
# 觀察：
...
#  則表示我們成功block住host3了
# 方法：
#  判斷port3 的tx 若大於我們指定的資料量，則將其mac addr block 掉（新增priority=2 的blocked flow entry）
# 未來：
#  增加時間的參數進去，(tx_after - tx)/ time > k 時，block掉
...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def _port_stats_reply_handler(self, ev):
        ...
        for stat in sorted(body, key=attrgetter('port_no')):
            ...
            if (stat.port_no == 3) and (stat.tx_bytes >= 5000):
                self.bbb()
                msg = ev.msg
